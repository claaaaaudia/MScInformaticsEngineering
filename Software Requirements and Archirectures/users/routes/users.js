...
router.delete("/:user", function (req, res, next) {
  ...
  User.deleteAccount(req.params.user, password)
    .then((result) => {
      res.sendStatus(204);
    })
    .catch((err) => {
      console.error("DELETE /users/:user error:", err && err.message ? err.message : err, err && err.stack ? err.stack : "");
      const msg = err && err.message ? err.message : "Error deleting user's information";
      if (msg === "User not found.") return res.status(404).jsonp(msg);
      if (msg === "Invalid password.") return res.status(401).jsonp(msg);
      return res.status(705).jsonp(msg);
    });
});
