...
/**
 * Delete a user
 * @body { "password": String (Required for account deletion) }
 * @returns Empty
 */
router.delete("/:user", auth.checkToken, function (req, res, next) {
  axios({
    method: "delete",
    url: usersURL + `${req.params.user}`,
    httpsAgent: httpsAgent,
    headers: {
      Authorization: req.headers["authorization"],
      "Content-Type": "application/json",
    },
    data: req.body, // Pass password in request body
  })
    .then((_) => res.sendStatus(204))
    .catch((err) => {
      const status = err.response?.status || 500;
      const message = err.response?.data || "Error deleting user";
