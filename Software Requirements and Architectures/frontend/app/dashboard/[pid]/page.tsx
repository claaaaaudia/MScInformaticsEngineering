"use client";
import ProjectShareCard from "@/components/project-page/ProjectShareCard";
...
export default function Project({
  ...
  // Timer para mostrar botão de cancelamento após 10 segundos
  useEffect(() => {
    if (!processing) {
  ...
  // Listener para cancelamento via socket
  useEffect(() => {
    let active = true;

    if (active && socket.data) {
      socket.data.on("process-cancelled", () => {
        if (active) {
          setProcessing(false);
          setProcessingProgress(0);
          setProcessingSteps(1);
          setShowCancelButton(false);
          // Restore previous sidebar state if we saved one
          if (prevSidebarOpen !== null && !isMobile) {
            sidebar.setOpen(prevSidebarOpen);
            setPrevSidebarOpen(null);
          }
          toast({
            title: "Processing cancelled",
            description:
              "Project processing has been cancelled. State restored.",
          });
        }
      });
    }
    ...
    return () => {
      active = false;
      if (socket.data) socket.data.off("process-cancelled");
    };
  }, [socket.data, toast, prevSidebarOpen, sidebar, isMobile]);
          ...
          <div className="flex items-center justify-between w-full xl:w-auto gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end xl:justify-normal w-full xl:w-auto">
              ...
              <div className="flex items-center gap-2">
                <ProjectShareCard projectId={project.data._id} initialToken={project.data.shareToken} />
              </div>
              {mode !== "results" && (
                <>
                  <Button
                      ...
                      processProject.mutate(
                        ...
                        },
                        {
                          onSuccess: () => {
                            setProcessing(true);
                            if (!isMobile) {
                              setPrevSidebarOpen(sidebar.open);
