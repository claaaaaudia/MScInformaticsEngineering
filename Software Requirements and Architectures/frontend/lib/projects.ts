...
export const updateProjectToolsOrder = async ({
  uid,
  pid,
  token,
  tools,
  ...
  pid: string;
  token: string;
  tools: { _id: string; position: number; procedure: string; params: any }[];
}) => {
  const response = await api.post(
  ...
  if (response.status !== 201) throw new Error("Failed to update tools order");
};

export const copySharedProject = async (token: string, authToken: string) => {
  const response = await api.post(
    `/projects/share/${token}/copy`,
    {},
    {
      headers: {
...
// Add this to your projects API file
export const accessSharedProject = async (token: string, authToken: string) => {
  const response = await api.post(
