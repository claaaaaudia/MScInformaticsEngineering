...
const {
  ...
  read_msg,
  send_msg_client_to_collaborators
...
function process_msg() {
  read_msg(async (msg) => {
    ...
    if (!/preview/.test(msg_id)) {
  send_msg_client_to_collaborators(user_msg_id, timestamp, process.project_id, process.user_id);
}
...
router.post("/:user/:project/process", (req, res, next) => {
  ...
  Project.getOne(req.params.user, req.params.project)
    .then(async (project) => {
      console.log("Processing project:", req.params.project);
      console.log("Project tools:", JSON.stringify(project.tools));
      console.log("Tools length:", project.tools?.length);
...
// Update a specific project by share token (edit permission required)
router.put("/share/:token/project", (req, res, next) => {
  Project.getOneByToken(req.params.token)
    .then(async (project) => {
      if (!project)
        return res.status(403).json({ error: "Invalid or revoked share link" });

      const permission = await Project.getPermissionByToken(req.params.token);
      if (permission !== "edit")
      ...
      Project.updateByToken(req.params.token, updatedProject)
        .then((_) => res.sendStatus(204))
        .catch((_) =>
          res.status(503).json({ error: "Error updating project information" })
        );
    })
    .catch((_) => res.status(500).json({ error: "Error acquiring project" }));
});

// Update a tool in a project by share token (edit permission required)
router.put("/share/:token/tool/:tool", (req, res, next) => {
  Project.getOneByToken(req.params.token)
    .then(async (project) => {
      if (!project)
        return res.status(403).json({ error: "Invalid or revoked share link" });
      ...
      try {
        ...
        Project.updateByToken(req.params.token, project.toObject())
          .then((_) => res.sendStatus(204))
          .catch((_) =>
            res.status(503).json({ error: "Error updating project information" })
          );
...
router.post("/:user/:project/cancel", (req, res, next) => {
  // Delete all active processes for this project and restore original images
  Project.getOne(req.params.user, req.params.project)
    .then(async (project) => {
      try {
        // Get all active processes for this project
        ...
        if (processes.length === 0) {
          res.status(400).jsonp("No active processing to cancel");
          return;
        }
        ...
        const source_path = `/../images/users/${req.params.user}/projects/${req.params.project}/src`;
        const preview_path = `/../images/users/${req.params.user}/projects/${req.params.project}/preview`;
        ...
        if (fs.existsSync(path.join(__dirname, preview_path)))
          fs.rmSync(path.join(__dirname, preview_path), {
            recursive: true,
            force: true,
          });

        // Send cancellation message to client via WebSocket
        const timestamp = new Date().toISOString();
        send_msg_client(
          `cancel-process-${uuidv4()}`,
          timestamp,
          req.params.user
        );

        res.status(200).jsonp("Processing cancelled successfully");
      } catch (error) {
...
router.post("/:user/:project/apply", (req, res, next) => {
  Project.applyResults(req.params.user, req.params.project)
    .then(() => res.sendStatus(204))
    .catch((err) => {
      console.error("Error applying results:", err);
      res.status(500).jsonp(err.message || "Error applying results");
    });
});

router.get("/:user/:project", async (req, res) => {
  try {
    console.log(`Getting project ${req.params.project} for user ${req.params.user}`);
    
    const project = await ProjectModel.findById(req.params.project);
    ...
    console.log(`Found project, owner: ${project.user_id}, collaborators:`, project.collaborators);

    const isOwner = project.user_id.toString() === req.params.user;
    const isCollaborator =
      project.collaborators &&
      project.collaborators.some((c) => c.toString() === req.params.user);

    console.log(`isOwner: ${isOwner}, isCollaborator: ${isCollaborator}`);

    if (!isOwner && !isCollaborator) {
      console.log("No access");
      return res.status(403).jsonp("No access to this project");
    }
    ...
    const response = {
      ...
      tools: project.tools,
      imgs: [],
      isCollaborative: project.collaborators && project.collaborators.length > 0,
      isOwner,
    };
    ...
    try {
      ...
      if (link) response.shareToken = link.token;
    } catch {
      // ignore share link lookup errors
    }

    console.log("Returning project successfully");
    res.status(200).jsonp(response);
  } catch (err) {
    console.error("FULL ERROR in GET /:user/:project:", err);
    res.status(500).jsonp("Error getting project");
  }
});
...
router.get("/:user", async (req, res, next) => {
  try {
    const projects = await Project.getAllAccessible(req.params.user);
    ...
    for (let p of projects) {
      ans.push({
        _id: p._id,
        name: p.name,
        isOwner: p.user_id.toString() === req.params.user,
        isCollaborative: p.collaborators && p.collaborators.some(c => c.toString() === req.params.user)
      });
    }

    res.status(200).jsonp(ans);
  } catch (_) {
    res.status(500).jsonp("Error acquiring user's projects");
  }
});
...
router.post("/:user/:project/tool", async (req, res, next) => {
  ...
  axios
    ...
    .then(async (resp) => {
      ...
      const canAccess = await Project.canAccess(req.params.user, req.params.project);
      ...
      Project.updateOne({ _id: req.params.project }, project)
        .then((_) => res.sendStatus(204))
        .catch((_) =>
          res.status(503).jsonp(`Error updating project information`)
        );
...
router.put("/:user/:project", async (req, res, next) => {
  try {
    ...
    await Project.updateOne({ _id: req.params.project }, project);
    res.sendStatus(204);
  } catch (_) {
